...
insertTree tree@(Node prio1 key1 children1) trees = case trees of
    [] -> [tree]
...
insert prio key heap = BinomialHeap {size = size heap + 1, trees = insertTree (Node prio key []) (trees heap)}
...
findMin heap = case trees heap of
    [] -> Nothing
    _ -> Just (minimumBy (compare `on` fst) [(prio t, key t) | t <- trees heap, isTree t])
    where isTree EmptyTree = False
          isTree _ = True
...
mergeTrees trees1 trees2 = snd acc ++ fst acc
    where acc = mapAccumL mergeTrees' trees1' trees2'
          unzippedTrees = unzip $ zipExtend EmptyTree EmptyTree trees1 trees2
          trees1' = fst unzippedTrees
          trees2' = snd unzippedTrees
          mergeTrees' (t1 : ts1) t2
            | isEmptyTree t1 || isEmptyTree t2 = (ts1, attach t1 t2)
            | otherwise = (insertTree (attach t1 t2) ts1, EmptyTree)
...
merge heap1 heap2 = BinomialHeap {size = size heap1 + size heap2, trees = mergeTrees (trees heap1) (trees heap2)}
...
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder [] = []
isolate placeHolder list = isolate_aux placeHolder [] list
  where
    isolate_aux placeHolder crt rest@(first : list) = case list of
      [] -> [(first, crt)]
      _  -> (first, crt ++ (placeHolder : list)) : isolate_aux placeHolder (crt ++ [first]) list
...
-}
removeMin heap@(BinomialHeap _ ts) =
  case findMin heap of
    Nothing -> heap
    Just (minPrio, minKey) ->
      let (before, Node _ _ orphans:after) = span (\t -> prio t /= minPrio || key t /= minKey) ts
          newTrees = reverse before ++ reverse orphans ++ after
      in BinomialHeap (size heap - 1) newTrees
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    show EmptyTree = "*"
    show (Node prio key children) = showNode 0 (Node prio key children)
      where
        showNode indent (Node p k children) =
          indentSpaces indent ++ show p ++ " (" ++ show k ++ ")" ++ childrenString children
          where
            childrenString [] = ""
            childrenString xs = '\n' : intercalate "\n" (map (showNode (indent + 2)) xs)

        indentSpaces n = replicate n ' '
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
    show heap = 
      case heap of
        BinomialHeap _ [] -> ""
        BinomialHeap _ trees -> intercalate "\n" $ map show trees
...
instance Functor (BinomialTree p) where
    ...
    fmap f (Node p k children) = Node p (f k) (fmap (fmap f) children)
...
instance Functor (BinomialHeap p) where
    ...
    fmap f (BinomialHeap size trees) = BinomialHeap size (map map_on_tree trees)
      where
        map_on_tree EmptyTree = EmptyTree
        map_on_tree (Node p k children) = Node p (f k) (map map_on_tree children)
